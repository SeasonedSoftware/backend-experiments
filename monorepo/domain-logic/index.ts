...
export const onResult = (onSuccess: (r: any) => any, onError: (r: any) => any, r: Result) => {
  console.log({ r })
...
const mutation = (action: (input: any) => ActionResult, parser?: ZodTypeAny) =>
({
  mutation: true,
  parser,
  action
...
export const tasks: Actions = {
  "post": mutation(async (input: z.infer<typeof taskCreateParser>) => success(await prisma.task.create({ data: input })), taskCreateParser),
  "get": query(async () => success(await prisma.task.findMany())),
  "delete": mutation(async (input: z.infer<typeof taskDeleteParser>) => success(await prisma.task.delete({
    where: input,
  })), taskDeleteParser),
  "put": mutation(async (input: z.infer<typeof taskUpdateParser>) => success(await prisma.task.update({
    where: { id: input.id },
    data: input
  })), taskUpdateParser),
